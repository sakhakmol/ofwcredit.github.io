...
if (SpeechRecognition = root.SpeechRecognition || root.webkitSpeechRecognition || root.mozSpeechRecognition || root.msSpeechRecognition || root.oSpeechRecognition,
  SpeechRecognition && voice_command) {
      $.root_.on("click", '[data-action="voiceCommand"]', function(a) {
          $.root_.hasClass("voice-command-active") ? $.speechApp.stop() : ($.speechApp.start(),
      ...
      $.speechApp = function(a) {
          return a.start = function() {
              ...
              smartSpeechRecognition ? (smartSpeechRecognition.start(),
              $.root_.addClass("voice-command-active"),
              $.speechApp.playON(),
              voice_localStorage && localStorage.setItem("sm-setautovoice", "true")) : alert("speech plugin not loaded")
          }
          ,
          a.stop = function() {
              smartSpeechRecognition && (smartSpeechRecognition.abort(),
  ...
  (function(a) {
      ...
      if (!SpeechRecognition)
          return root.smartSpeechRecognition = null,
          a;
      var b, c, d = [], e = {
          "start": [],
          "error": [],
      ...
      }, f = 0, g = /\s*\((.*?)\)\s*/g, h = /(\(\?:[^)]+\))\?/g, i = /(\(\?)?:\w+/g, j = /\*\w+/g, k = /[\-{}\[\]+?.,\\\^$|#]/g, l = function(a) {
          return a = a.replace(k, "\\$&").replace(g, "(?:$1)?").replace(i, function(a, b) {
              return b ? a : "([^\\s]+)"
          }).replace(j, "(.*?)").replace(h, "\\s*$1?\\s*"),
          new RegExp("^" + a + "$","i")
      }, m = function(a) {
      ...
      root.smartSpeechRecognition = {
          "init": function(g, h) {
              ...
              b.onstart = function() {
                  m(e.start),
                  debugState && (root.console.log("%c \u2714 SUCCESS: User allowed access the microphone service to start ", debugStyle_success),
                  root.console.log("Language setting is set to: " + b.lang, debugStyle)),
              ...
              b.onerror = function(a) {
                  ...
                  case "service-not-allowed":
                      ...
                      debugState && root.console.log("%c WARNING: Microphone was not detected (either user denied access or it is not installed properly) ", debugStyle_warning),
                      m((new Date).getTime() - f < 200 ? e.errorPermissionBlocked : e.errorPermissionDenied)
                  }
              }
              ,
              b.onend = function() {
              ...
              b.onresult = function(a) {
                  ...
                  for (var b, c = a.results[a.resultIndex], f = 0; f < c.length; f++) {
                      ...
                      debugState && root.console.log("Speech recognized: %c" + b, debugStyle);
                      for (var g = 0, h = d.length; h > g; g++) {
                          var i = d[g].command.exec(b);
                          if (i) {
                              var j = i.slice(1);
                              debugState && (root.console.log("command matched: %c" + d[g].originalPhrase, debugStyle),
          ...
          "removeCommands": function(b) {
              return b === a ? void (d = []) : (b = Array.isArray(b) ? b : [b],
